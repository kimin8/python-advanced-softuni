...
class BaseInfluencer(ABC):
    ...
    def display_campaigns_participated(self) -> str:
        ...
        if self.campaigns_participated:
            ...
            message = f"{type(self).__name__} :) {self.username} :) participated in the following campaigns:{new_line}"
            messages_list = []
            for campaign in self.campaigns_participated:
                messages_list.append(f"  - Campaign ID: {campaign.campaign_id}, Brand: {campaign.brand}, Reached followers: {self.reached_followers(campaign.__class__.__name__)}")
            message += new_line.join(messages_list)
